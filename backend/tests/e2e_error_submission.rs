...
// Full end-to-end test: create project via API, submit an error, verify persistence.
#[tokio::test]
#[ignore]
async fn test_submit_error_end_to_end() {
    // Require TEST_DATABASE_URL for integration run
    let db_url = match std::env::var("TEST_DATABASE_URL") {
        Ok(v) => v,
        Err(_) => {
            eprintln!("Skipping e2e test: TEST_DATABASE_URL not set");
            return;
        }
    };

    // Setup DB and apply migrations
    ...
    // Start test app bound to test pool
    ...
    // 1) Create project using a signed JWT (real auth flow)
    ...
    let sub = "e2e-user-011".to_string();
    ...
    let req = test::TestRequest::post()
        ...
        .set_json(&json!({"name": "E2E Project"}))
        .to_request();
    ...
    // 2) Submit an error with returned API key
    let raw_error = json!({
    ...
    assert_eq!(j.get("was_duplicate").and_then(|v| v.as_bool()).unwrap_or(true), false);
    assert_eq!(j.get("count").and_then(|v| v.as_i64()).unwrap_or(0), 1);
}
