...
// E2E: simulate rate limit exceeded by pre-populating rate_limit_tracker
#[tokio::test]
#[ignore]
async fn test_rate_limiting_end_to_end() {
    let db_url = match std::env::var("TEST_DATABASE_URL") {
        Ok(v) => v,
        Err(_) => {
            eprintln!("Skipping e2e rate limit test: TEST_DATABASE_URL not set");
            return;
        }
    };
    ...
    let sub = "e2e-user-033".to_string();
    ...
    let req = test::TestRequest::post()
        ...
        .set_json(&json!({"name": "Rate Project"}))
        .to_request();
    ...
    // Pre-populate rate_limit_tracker with count >= 10000 for current hour
    let now = Utc::now();
    ...
    sqlx::query(
        "INSERT INTO rate_limit_tracker (project_id, hour_window, error_count, updated_at)
         ...
         ON CONFLICT (project_id) DO UPDATE SET error_count = 10000, updated_at = NOW()"
    )
    .bind(project_id)
    .bind(hour_window)
    .execute(&pool)
    .await
    .expect("insert rate row");
    ...
    // Expect HTTP 429 Too Many Requests
